...
def main():
    ...
    cf.data.reindex_dataframe(index_as_liste=new_index)
    
    # columns name in the excel file
    # c Chichawa_Hr_(%)	Chichawa_Rg_(W/m2)	Chichawa_Tair_(°C)	Chichawa_Vv_(m/s)
    # DateBis	R3_hr_(%)	R3_Rg_(W/m2)	R3_Tair_(°C)	R3_vv_(m/s)	R3_P30(mm)
    # Date	Ouka_1_Hr_(%)	Ouka_1_Rg_(W/m2)	Ouka_1_Tair_(°C)	Ouka_1_Vv_(m/s)
    # Date	Aremd_Hr_(%)	Aremd_Rg_(W/m2)	Aremd_Tair_(°C)	Aremd_IVv_(m/s)
    ...
    # Oukaimeden	31.18205	-7.86553

    cf.fill('ta', -7.91898, 31.13003)
    cf.fill('rs', -7.91898, 31.13003)
    cf.fill('rh', -7.91898, 31.13003)
    cf.fill('ws', -7.91898, 31.13003)
    print(cf.data.missing_data_statistics())
    cf.data.export("data/armed_full.csv", index=True)
