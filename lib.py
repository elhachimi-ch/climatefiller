...
class Lib:
    ...
    def et0_penman_monteith_hourly(row):
        ...
        ta, rh, u2, rs, lat, elevation, doy = row['ta'], row['rh'], row['u2'], row['rg'], row['lat'], row['elevation'], row['doy']
        ...
        # convert units
        rs *= 0.0864  # convert watts per square meter to megajoules per square meter 0.0288 = 60x60x8hours or 0.0864 for 24 hours
        ta_mean = ta / 2
        ta_max_kelvin = ta + 273.16  # air temperature in Kelvin
        ta_min_kelvin = ta + 273.16  # air temperature in Kelvin
        ...
        # Calculate net longwave radiation
        rnl = SIGMA * (((math.pow(ta_max_kelvin, 4) + math.pow(ta_min_kelvin, 4)) / 2) * (0.34 - (0.14 * math.sqrt(ea))) * ((1.35 * (rs / rso)) - 0.35))
        ...
        et0 = ((0.408 * delta * (rn - G)) + gamma * ((37 / (ta_mean + 273)) * u2 * (es - ea))) / (delta + (gamma * (1 + 0.34 * u2)))
